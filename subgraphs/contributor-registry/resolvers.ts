...
async function loadAll(reactorClient: BaseSubgraph["reactorClient"]): Promise<{
  ...
  for (const c of campaigns) {
    const parents = await reactorClient.getIncomingRelationships(c.header.id, "drive/child");
  ...
  for (const p of pledges) {
    const parents = await reactorClient.getIncomingRelationships(p.header.id, "drive/child");
    const drive = parents.results.at(0);
